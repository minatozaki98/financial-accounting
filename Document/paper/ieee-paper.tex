%% IEEE Conference paper draft - 6 pages
...
\title{Evaluating ChatGPT (Codex 5.4) for Security, Performance, and Code-Quality Improvement of a Financial Accounting REST API: A Branch-Isolated Baseline Study\\
{\footnotesize \textsuperscript{}}
\thanks{Preprint, March 2026. This work evaluates AI-assisted improvements on a research ASP.NET Core REST API; it is not affiliated with any production financial system.}}

\author{\IEEEauthorblockN{Zaw Ye Htut Ko}
\IEEEauthorblockA{\textit{Graduate Program in Information Technology} \\
\textit{Assumption University}\\
Bangkok, Thailand \\
kozawyehtutko43@gmail.com}
\and
\IEEEauthorblockN{Advisor: Paitoon Porntrakoon}
\IEEEauthorblockA{\textit{Graduate Program in Information Technology} \\
\textit{Assumption University}\\
Bangkok, Thailand}
}
...
\textbf{What is SonarQube?} SonarQube is an open-source static-analysis platform that scans source code for \emph{bugs} (probable defects), \emph{vulnerabilities} (known security weaknesses), \emph{code smells} (maintainability issues), \emph{security hotspots} (suspicious patterns flagged for manual review), test coverage, and code duplication. Findings are emitted with rule IDs (e.g., \textsc{S125}, \textsc{S6966}), severities, and human-readable remediation guidance.
...
\toprule
Profile & p95 Before & p95 After & $\Delta$ (\%) & Err After & Gate \\
\midrule
...
ChatGPT (Codex 5.4) is the \emph{subject} of this study and was used to generate candidate patches for the three fix branches. All patches were reviewed, tested, and accepted or rejected by the author. Language polishing of this manuscript used an LLM for proofreading only; all scientific claims, measurements, tables, and figures were produced and verified manually from the experimental artifacts.
