...
const expectedApiBaseUrl = process.env.EXPECTED_API_BASE_URL ?? "http://localhost:5296";
...
test("research demo shell exposes login and evidence workflow", async ({ page }) => {
  ...
  await expect(page.getByLabel(/API base URL/i)).toHaveValue(expectedApiBaseUrl);
});
...
test.describe("live API research workflow", () => {
  ...
  test("admin can sign in, run a report, and open audit logs", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /View balance for/i }).first().click();
