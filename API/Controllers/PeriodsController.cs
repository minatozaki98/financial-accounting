...
namespace API.Controllers
{
    ...
    public class PeriodsController : ControllerBase
    {
        ...
        [HttpGet("{periodId:int}/close-preview")]
        public async Task<IActionResult> PreviewClose(
            [FromRoute] int periodId,
            CancellationToken cancellationToken)
        {
            var result = await _accountingPeriodService.PreviewCloseAsync(periodId, cancellationToken);
            return Ok(result);
        }
        ...
        public async Task<IActionResult> Close(
            [FromRoute] int periodId,
            [FromBody(EmptyBodyBehavior = EmptyBodyBehavior.Allow)] CloseAccountingPeriodRequestDto? request,
            CancellationToken cancellationToken)
        {
            var actorId = UserClaimsHelper.GetUserId(User);
            if (!actorId.HasValue)
            ...
            if (request == null)
            {
                var closed = await _accountingPeriodService.CloseAsync(
                    periodId,
                    actorId.Value,
            ...
            var result = await _accountingPeriodService.CloseAsync(
                periodId,
                request,
                actorId.Value,
                HttpContext.Connection.RemoteIpAddress?.ToString(),
