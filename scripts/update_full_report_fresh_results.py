...
def update_report(report_path: Path, root: Path = ROOT) -> None:
    ...
    if any(
        marker in abstract.text
        for marker in (
            "The current reproduction provides the primary results.",
            "Fresh reproduction runs executed on September 22, 2026 provide the primary results.",
        )
    ):
        return
    ...
    replace_paragraph(
        ...
        "The findings are limited to one ASP.NET Core API, one C# financial-accounting codebase, one AI assistant configuration, one Windows 11 host, and one fresh reproduction per evidence branch. The fresh run records identify the local database and tool versions but do not preserve the host CPU model or core count, installed RAM, per-run CPU and memory utilization, or resource limits assigned to the application, database, and Docker tools. No managed-cloud service tier or dedicated production capacity was evaluated. Contention, memory pressure, and different CPU or RAM allocations could change latency and throughput. The fresh JMeter result illustrates the boundary: soak and spike aggregates improved while the 100 and 500 VU core p95 thresholds failed. The study cannot attribute that difference to hardware or generalize it to another device or service tier. Future runs should record hardware specifications and utilization, fix resource allocations, and repeat matched workloads across specified tiers with variability or confidence intervals. Historical measurements remain in the appendices for provenance; Chapters 4 and 5 use the fresh reproduction as the current result. Future work should also compare multiple LLMs and test production-like HTTPS deployment to evaluate the remaining ZAP HTTP Only Site observation.",
    )
