// Generate an anonymous InCIT 2026 IEEE-style conference paper as a .docx.
// Run: node scripts/generate-incit2026-ieee-docx.js
const fs = require("fs");
...
const OUT = "Document/paper/incit2026-ieee-submission.docx";
...
const body = (children, opts = {}) => new Paragraph({
  ...
  spacing: { after: 120 },
  indent: opts.firstLine === false ? undefined : { firstLine: 288 },
  ...opts,
...
const workflowFigure = () => [
  ...WORKFLOW_PANELS.map(([file, title]) => new Paragraph({
    ...
    children: [new ImageRun({
      ...
      transformation: { width: 232, height: 146 },
...
const authors = new Paragraph({
  ...
  spacing: { after: 120 },
  children: [
    new TextRun({ text: "Zaw Ye Htut Ko, Darun", font: FONT, size: SIZE_AUTHOR }),
  ],
});
...
const abstractText = new Paragraph({
  alignment: AlignmentType.JUSTIFIED,
  indent: { left: 360, right: 360, firstLine: 272 },
  spacing: { after: 200 },
  children: [new TextRun({
...
const results = [
  ...
  leadPara("What is OWASP ZAP?", "OWASP Zed Attack Proxy (ZAP) is a free, open-source dynamic application security testing (DAST) tool. It performs a passive baseline scan that observes responses to crawled requests for missing security headers, cookie problems, and known-vulnerable bundled JavaScript libraries [18]; and an active API scan that, given an OpenAPI definition and a valid authentication token, exercises every endpoint and probes for OWASP Top-10–style flaws [17], [19]. Findings are emitted by risk level: High, Medium, Low, Informational."),
  leadPara("Why this dimension?", "APIs in financial domains are routinely targeted because they handle money, identity, and audit-grade records; OWASP's API risk taxonomy highlights authorization, authentication, sensitive business-flow, and security-misconfiguration failures as core API security risks [17]. A vulnerability that survives into production can impose substantial incident-response and remediation costs; IBM's 2025 breach-cost report estimates the global average breach cost at USD 4.4 million [22]. DAST tools such as ZAP catch real-world exposures (missing CSP, weak CORS, vulnerable dependencies) that static analysis cannot see, and they do so on the running service rather than on the source tree [18], [19]."),
...
const discussion = [
  ...
  body([
    ...
    run("SonarQube rules themselves are imperfect [15]; ZAP’s informational alerts blur the line between signal and noise [18], [19]; JMeter’s mixed-workload drift heuristic is sensitive to workload composition [20]. We pre-register the gates to avoid post-hoc tuning, and we publish the full configuration so that third parties can re-run on their own branch."),
  ]),
];
...
const disclosure = [
  ...
  para("ChatGPT (Codex 5.4) is the subject of this study and was used to generate candidate patches for the three fix branches. The specific sections that report AI-assisted work are the methodology, experiments, results, and discussion sections where the LLM-guided workflow and candidate patches are described. All patches were reviewed, tested, and accepted or rejected by the authors. Language polishing of this manuscript used an LLM for proofreading only; all scientific claims, measurements, tables, and figures were produced and verified from the experimental artifacts."),
];
...
const refs = [
  ...
  "[16] SonarSource, “Rules overview,” SonarQube Server Documentation. Accessed: May 30, 2026. [Online]. Available: https://docs.sonarsource.com/sonarqube-server/10.4/user-guide/rules/overview",
  "[17] OWASP Foundation, “OWASP Top 10 API Security Risks--2023,” OWASP API Security Project, 2023. Accessed: May 30, 2026. [Online]. Available: https://owasp.org/API-Security/editions/2023/en/0x11-t10/",
  "[18] OWASP ZAP, “ZAP--Baseline Scan,” ZAP Documentation. Accessed: May 30, 2026. [Online]. Available: https://www.zaproxy.org/docs/docker/baseline-scan/",
  "[19] OWASP ZAP, “ZAP--API Scan,” ZAP Documentation. Accessed: May 30, 2026. [Online]. Available: https://www.zaproxy.org/docs/docker/api-scan/",
  "[20] Apache Software Foundation, “Apache JMeter User's Manual: Glossary,” Apache JMeter Documentation. Accessed: May 30, 2026. [Online]. Available: https://jmeter.apache.org/usermanual/glossary.html",
  "[21] B. Beyer, C. Jones, J. Petoff, and N. R. Murphy, Eds., “Service Level Objectives,” in Site Reliability Engineering: How Google Runs Production Systems. Sebastopol, CA, USA: O'Reilly Media, 2016. [Online]. Available: https://sre.google/sre-book/service-level-objectives/",
  "[22] IBM Security and Ponemon Institute, “Cost of a Data Breach Report 2025,” IBM, 2025. Accessed: May 30, 2026. [Online]. Available: https://www.ibm.com/reports/data-breach",
];
  ...
  sections: [
    {
      properties: {
        page: {
          ...
          margin: { top: 1080, right: 900, bottom: 1440, left: 900, header: 720, footer: 720 },
        },
      },
      children: topMatter,
    },
